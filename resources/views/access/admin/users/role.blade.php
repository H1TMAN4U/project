    ...
    <div class="bg-gray-100 flex flex-col border border-gray-200 rounded dark:bg-gray-800 dark:border-gray-700">
        <div class="flex flex-row items-center px-2 justify-between border-b border-gray-200 dark:border-gray-700">
            <a href="{{ route('admin.users.index') }}">
                <button
                    class="bg-gray-200 border border-gray-100 hover:bg-gray-300 text-gray-600 px-4 py-1 rounded
    ...
    <div class="flex flex-col items-center justify-center mt-1 gap-1 ">
        <div class="w-full p-4 sm:p-8 bg-gray-50 dark:bg-gray-800 shadow sm:rounded border border-gray-200 dark:border-gray-700">
            ...
            <div class="flex space-x-2 py-2">
                @if ($user->roles)
                    @foreach ($user->roles as $user_role)
                        <form class="px-4 py-2 bg-blue-600 hover:bg-red-600 text-white rounded transition duration-500 ease-out-in
            ...
            <form method="POST" action="{{ route('admin.users.roles', $user->id) }}">
                ...
                <select id="role" name="role" autocomplete="role-name"
                    class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600">
        ...
        <div class="w-full p-4 sm:p-8 bg-gray-50 dark:bg-gray-800 shadow sm:rounded border border-gray-200 dark:border-gray-700">
            ...
            <div class="flex space-x-2 p-2">
                @if ($user->permissions)
                    @foreach ($user->permissions as $user_permission)
                        <form class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded" method="POST"
            ...
            <form method="POST" action="{{ route('admin.users.permissions', $user->id) }}">
                @csrf
                <label for="permission" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    Permission Name
                </label>
